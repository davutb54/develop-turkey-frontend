  ...
  return (
    <>
      <nav style={{ backgroundColor: navBgColor, transition: 'background-color 0.3s ease-in' }} className={`shadow-md border-b ${isCustomTheme ? 'border-transparent' : 'border-gray-100'} relative z-[40]`}>
        ...
        {/* --- MOBİL AÇILIR MENÜ (DROPDOWN) - GİRİŞ YAPILMAMIŞ --- */}
        {!user && !loading && isMobileMenuOpen && (
          <div className="lg:hidden absolute top-full left-0 w-full bg-white shadow-xl border-t border-gray-100 py-2 animate-fade-in-down z-50">
            <div className="flex flex-col py-2">
              <Link to="/login" onClick={() => setIsMobileMenuOpen(false)} className="px-5 py-3 text-sm font-bold text-gray-700 hover:bg-gray-50 hover:text-blue-600 flex items-center gap-3">
              ...
              <Link to="/register" onClick={() => setIsMobileMenuOpen(false)} className="px-5 py-3 text-sm font-bold text-gray-700 hover:bg-gray-50 hover:text-blue-600 flex items-center gap-3">
        ...
        {/* --- MOBİL AÇILIR MENÜ (DROPDOWN) - GİRİŞ YAPILMIŞ --- */}
